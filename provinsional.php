...
$p_vote = "";
 if(isset($_POST['pro_btn']))
        {
            if(isset($_POST['pro_drop_down']))
            {
                $p_vote = $_POST['pro_drop_down'];
                
                $query5 = 'select u_id from user where email ="'.$_SESSION['email'].'"';
                $result2 = mysql_query($query5) or die(mysql_error());
                
                $row2 = mysql_fetch_array($result2); 
                
                $query6 = 'insert into u_votes(u_id,p_vote) values("'.$row2['u_id'].'","'.$p_vote.'")';
            ...
            <div class="content">
                ...
                <?php
                    ...
                    $u_id = $_SESSION['u_id'];
                     ...
                     if($dn==0)
                     {
                         ?><form method="POST" name="provinsial" action="provinsional.php"><?php
                        ...
                        <input type="submit" value="VOTE" name="pro_btn" />
                        </form><?php
                     }
                     else
                     {
                         print "<h1>YOU ALREADY VOTED PROVINSIONALY</h1>";
                         print $mess;
                     }
