...
if(isset($_POST['update']))
 {
      if(isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['age']) && isset($_POST['province']) &&  isset($_POST['email']) && isset($_POST['password']) && isset($_POST['confirm_pass']) )
      {
        ...
        $query = 'update user set first_name="'.$firstname.'", last_name="'.$lastname.'",age="'.$age.'",province="'.$province.'",email="'.$email.'",password="'.$password.'",confirm_pass="'.$confirm_pass.'"  where email = "'.$_SESSION['email'].'"';
        $test = mysql_query($query) or die(mysql_error());
           
        if($password == $confirm_pass)
        {
            $test = mysql_query($query) or die(mysql_error());if($test);
            if($test)
            {        if($password == $confirm_pass)

                $mess = "<h1>INFORMATION SUCCESSFULLY UPDATED</h1>";
            }
            else
            {
                $mess = "<h2>ENCOUNTERED AN ERROR WHILE UPDATING INFORMATION</h2>";
            }
        }
        else
        {
    ...
    <body>
        ...
        <div id="box">
                 ...
                 <?php 
                    if(isset($_SESSION['email']))
                    {
                        ?>
                            ...
                            <a href="provinsional.php">PROVINCIAL VOTE</a><br />
            ...
            <div class="content">
                    ...
                    ?>
                    <br />
                <form method="POST" name="UPDATE" action="profile.php">
                    ...
                    <p>Enter First name: </td><td> <input type="text" name="firstname" value="<?php print $row['first_name'];?>"  required/></p>
                    <p>Enter Last name:  </td><td><input type="text" name="lastname" value="<?php print $row['last_name'];?>"  required/></p>
                    <p>Enter Age:</td><td> <input type="text" name="age" value="<?php print $row['age'];?>"  required/></p>
                    <p>Enter Identity Number:  </td><td><strong><?php print $row['id_no'];?></strong></p>
                    <p>Select Province:  </p>
                            ...
                            </select><?php print $row['province'];?>
                    
                    
                    <h3>USER INFORMATION</h3>
                    
                    <p>Enter Email:  <input type="email" name="email" value="<?php print $row['email'];?>" required/></p>
                    <p>Enter Password: <input type="text" name="password" value="<?php print $row['password'];?>" required/></p>
                    <p>Confirm Password:  <input type="text" name="confirm_pass" value="<?php print $row['confirm_pass'];?>"  required/></p>
